...
namespace UnitTests
{
    public class ConcurrencyTests
    {
        ...
        {
            // Arrange
            Concurrency<int>.StartTransaction("key", TimeSpan.FromSeconds(1));

            // Act
            var task = Task.Run(() => Concurrency<int>.StartTransaction("key", TimeSpan.FromSeconds(10)));
            Concurrency<int>.EndTransaction("key");
